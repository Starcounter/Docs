...
* `AllowFromDirection`: Allows attachment rule calls from this URI (it defines if this URI can trigger attachment process). That means if an app calls `Self.GET()` on an attachment rule URI it will/will not request other apps for content with the same contexts. Default value is "True".
* `AllowToDirection`: Allows an attachment rule calls to this URI (it defines if this URI can be triggered during attachment process). That means if other app requests on attachment rule with the same contexts as one of the attachment rules described in your app, your app will/will not respond. Default value is "True".
