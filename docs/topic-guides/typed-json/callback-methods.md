...
The `OnData` method is usually used to initialize the parts of the view-model that cannot be initialized by setting the `Data` property. Several examples of this can be found in the [UniformDocs repo](https://github.com/Starcounter/UniformDocs/blob/master-2.4/src/UniformDocs/ViewModels/Components/ChartPage.json.cs).
