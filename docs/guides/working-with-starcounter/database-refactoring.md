...
Reloading the database dump after removing the field will fail. This issue is reported in issue 3929. Also, just doing an unload will fail if issued after the auto-migration have succeeded; reported in issue 3932.
...
Reloading the database dump after removing the property will fail. This issue is reported in issue 3929. Also, just doing an unload will fail if issued after the auto-migration have succeeded; reported in issue 3932.
...
To successfully unload after the auto-migration has occurred, the unload has to be carried out with the `--allowPartial` flag, i.e. `staradmin unload --allowPartial`. This issue is being discussed in issue 3933.
